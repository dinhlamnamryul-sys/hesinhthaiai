...
st.markdown("""
Upload Excel (.xlsx) hoặc Word (.docx). Hệ thống sẽ tự động nhận diện các cột:  
- Chủ đề  
- Nội dung  
- Mức độ  
- Số câu
...
def normalize_columns(df):
    ...
    for col in df.columns:
        ...
        if "chủ đề" in lc or "chude" in lc or "topic" in lc:
            ...
            col_map[col] = col  # giữ nguyên nếu không map
...
if uploaded_matrix:
    ...
    if missing_cols:
        st.error(f"❌ Không tìm thấy các cột quan trọng: {missing_cols}")
    else:
